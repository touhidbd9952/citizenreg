...
class Admincontroller extends CI_Controller {
	...
	public function check_admin_login()
	{
		if($_POST)
		{
			...
			{
				redirect('admincontroller/index?esk=Wrong username or password');
			}
			
			//for subadmin
			$admininfo = $this->db->query("Select * from t_admin where username = '".$username."' ")->result();
			if(count($admininfo) == 0)
			{  
				redirect('admincontroller/check_subadmin_login/'.$username.'/'.$password);
			}
			...
			if((string)$adminpass == (string)$password)
			{
				...
				$ses['adminname'] = 'Super admin';
				$ses['admintype'] = 'superadmin';
	...
	{
		$data = array();
		$data['admintype'] = "subadmin";
		$data['content'] = $this->load->view('admin_login_form');
	}
	public function check_subadmin_login($username,$password)
	{  
			...
			if($username == "" || $password=="")
			{
				redirect('admincontroller?esk=Wrong username or password');
			}
			
			$admindata = $this->db->query("Select * from t_subadmin where username = '".$username."' and accountstatus = 0 ")->result(); 
			$adminpass = ""; $adminid="";  $adminname="";
			foreach($admindata as $adm)
			{
				$adminpass = $adm->password;
				$adminid = $adm->id;
			...
			if((string)$adminpass == (string)$password)
			{
				...
				$ses['adminname'] = $adminname;
				$ses['admintype'] = 'subadmin';
	...
	{
		$data = array();
		$data['menu'] = 'admin_registration_form';
		$data['adminlist'] = $this->db->query("Select * from t_subadmin ")->result();
		$data['container'] = $this->load->view('admin_registration_form',$data,true);
		$this->load->view('masteradmin',$data);
	}
	public function create_admin()
	{ //adminname username password accountstatus
		if(!$_POST){redirect("admincontroller/admin_logout");}
		else
		{ 
			$adminname = trim($this->input->post("adminname"));
			$username = trim($this->input->post("username"));
			$password = trim($this->input->post("password"));
			$accountstatus = trim($this->input->post("accountstatus"));
			
			$err=0;
			$emsg="";
			if($adminname == ""){$emsg = ++$err.". Admin Name Required<br>";}
			if($username == ""){$emsg = ++$err.". Admin Username Required<br>";}
			else{
				$adminlist = $this->db->query("Select * from t_subadmin ")->result();
				foreach($adminlist as $adm)
				{
					if($adm->username == $username){$emsg = ++$err.". Username already exist<br>";}
				}
			}
			if($password == ""){$emsg = ++$err.". Admin Password Required<br>";}
			
			if($err==0)
			{
				$adminpass = $this->mm->insert_rc4_pass($username,$password); 
				$data = array();
				$data["adminname"] = $adminname;
				$data["username"] = $username;
				$data["password"] = $adminpass;
				$data["accountstatus"] = $accountstatus;
				$data["createdate"] = date('yy-m-d h:i:s'); 
				
				$id = $this->mm->Insert_data_getid('t_subadmin',$data);
				if($id !="")
				{
					redirect("admincontroller/admin_registration_form?sk= Created Successfully");
				}
				else
				{
					redirect("admincontroller/admin_registration_form?esk=".$emsg);
				}
	...
	public function view_admin_list()
	{
		...
		else
		{
			...
			$data['adminlist'] = $this->db->query("Select * from t_subadmin where id = '".$userid."' ")->result();
		}
		$data['container'] = $this->load->view('view_admin_list',$data,true);
		$this->load->view('masteradmin',$data);
	}
	public function admin_edit_form($id)
	{
		$data = array();
		$data['menu'] = 'admin_registration_form';
		$data['admininfo'] = $this->db->query("Select * from t_subadmin where id='".$id."' ")->result();
		$data['container'] = $this->load->view('admin_registration_edit_form',$data,true);
		$this->load->view('masteradmin',$data);
	}
	public function update_admin_info()
	{
		...
		{ 
			...
			if($err==0)
			{
				...
				$data["password"] = $adminpass;
				$data["accountstatus"] = $accountstatus;
				$data["updatedate"] = date('yy-m-d h:i:s'); 
				
				if($this->mm->update_info('t_subadmin',$data,array("id" =>$id)))
				{
					redirect("admincontroller/view_admin_list?sk= Updated Successfully");
				}
				else
				{
					redirect("admincontroller/admin_edit_form/".$id."?esk= Update error, try again");
				}
				
			}
			else
			{
				redirect("admincontroller/admin_edit_form/".$id."?esk=".$emsg);
			}
